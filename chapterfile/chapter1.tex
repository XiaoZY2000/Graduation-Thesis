...
information at inference time to enhance recommendation performance.

Importantly, the proposed DUP-OT framework does not rely on overlapping users or
items during training. User preferences are modeled as structured distributions
to enhance expressive capacity, while cross-domain knowledge transfer under
non-overlapping settings is achieved through optimal transport based on semantic
alignment. When overlapping user information becomes available at inference
time, it can be incorporated as an optional enhancement rather than a
prerequisite, allowing the model to flexibly adapt to different real-world
