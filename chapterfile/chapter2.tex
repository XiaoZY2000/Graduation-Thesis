...
behavioral signals rather than explicit ratings. Such feedback is typically
represented as abstract preference indicators or scores, and is commonly
modeled within a non-negative real-valued space, i.e.,
...
This objective is typically optimized using ranking-based loss functions, such
as pairwise losses (e.g., Bayesian Personalized Ranking) or listwise ranking
losses.
...
and formally define the fundamental settings of cross-domain recommendation tasks.
...
\begin{equation}
	...
	\lambda \, \mathcal{L}_{\text{transfer}}(\mathcal{R}_s, \mathcal{R}_t; \Theta_s, \Theta_t),
\end{equation}
...
The second term $\mathcal{L}_{\text{transfer}}$ captures the knowledge transfer between
...
The specific design of the transfer loss $\mathcal{L}_{\text{transfer}}$ can vary
...
To address the lack of overlapping entities, some recent studies have explored
distribution-level or structure-level knowledge transfer, shifting the focus
from aligning individual users or items to matching higher-level statistical
structures across domains.

Rather than relying on explicit entity correspondence, these approaches often
aim to:
\begin{itemize}
	\item capture population-level or structural regularities shared across domains,
	\item represent user preferences in a form amenable to cross-domain comparison,
	\item and perform alignment based on appropriate distance measures between such
	      representations.
\end{itemize}

This line of research motivates the use of probabilistic representations and
distributional distance measures in cross-domain recommendation, particularly
in settings where direct entity overlap is unavailable.

\subsection{Connection to This Thesis}
This thesis also considers the non-overlapping cross-domain recommendation
setting, where neither users nor items are shared across domains. However, the
primary motivation for adopting a distributional view of user preferences in
this work is to enhance the expressive capacity of user representations, by
capturing uncertainty and multi-faceted interest structures that are difficult
to model with single point embeddings.

Within this framework, distributional preference modeling additionally provides
a structured representation that can be effectively aligned across domains.
In particular, by modeling user preferences as Gaussian Mixture Models, the
resulting mixture components and weights offer a natural interface for applying
optimal transport, which serves as the core mechanism for cross-domain knowledge
transfer under non-overlapping settings.

In the following sections, we introduce Gaussian Mixture Models for modeling
user preference distributions, and Optimal Transport-based methods for aligning
these structured representations in a mathematically grounded manner.
...
mass between Gaussian components.
...
Let $\mu, \nu \in \mathcal{P}(\mathcal{Z})$ be two probability measures defined on
a metric space $(\mathcal{Z}, d)$.
The Kantorovich formulation of optimal transport seeks a transport plan
$\gamma \in \Pi(\mu, \nu)$ that minimizes the expected transportation cost:
\begin{equation}
\min_{\gamma \in \Pi(\mu, \nu)}
\int_{\mathcal{Z} \times \mathcal{Z}} d(z, z') \, d\gamma(z, z'),
\end{equation}
...
\left(
\min_{\gamma \in \Pi(\mu, \nu)}
\int_{\mathcal{Z} \times \mathcal{Z}} d(z, z')^p \, d\gamma(z, z')
\right)^{\frac{1}{p}}.
\end{equation}
