...
In this section, we briefly review representative recommender system techniques that form the foundation of modern recommendation models.
...
By abstracting away explicit item content, 
...
By user or item similarities, memory-based CF can be further categorized into user-based and item-based approaches. 
User-based CF recommends items to a user based on the preferences of similar users, while item-based CF suggests items similar to those the user has previously liked.
...
Their proposed method can be illustrated as in Figure~\ref{fig:WCDR}.

%这里放图
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/WCDR.pdf}
	\caption{Illustration of the WCDR framework~\cite{Zhu_Jin_Zhang_Meng_Zhang_Li_2024_WCDR}, which models users and items as probability distributions to capture uncertainty in overlapping cross-domain recommendation.}
	\label{fig:WCDR}
\end{figure}

WCDR models users and items as probability distributions to capture uncertainty in overlapping cross-domain recommendation.
By contrast, our work considers a more challenging non-overlapping CDR scenario without any shared users or items, where direct entity-level alignment is unavailable in the training stage and cross-domain transfer must rely on distribution-level modeling.
...
Beyond latent embedding mapping, several works investigate cross-domain knowledge transfer by aligning heterogeneous feature spaces.
Kusano and Oyamada~\cite{Kusano_Oyamada_2023_ATP} proposed ATP, which formulates non-overlapping cross-domain recommendation as an attribute transportation problem.
ATP focuses on scenarios where neither users nor attributes are shared across domains, and aims to estimate cross-domain user similarity by aligning heterogeneous attribute spaces.
Specifically, ATP leverages optimal transport to learn a soft correspondence between source and target domain attributes, enabling users to be re-described in a unified attribute space for similarity computation and recommendation.
By operating at the attribute level, ATP provides an interpretable and feature-driven solution for non-overlapping cross-domain recommendation.

Liu et al.~\cite{Liu_Chen_Liao_Hu_Tan_Wang_Zheng_Ong_2024_JPEDET} proposed JPEDET, a joint preference exploration and dynamic embedding transportation framework for non-overlapping cross-domain recommendation.
JPEDET addresses the absence of overlapped users and items by learning transferable user representations from both rating and review information, and further aligning user embeddings across domains through a dynamic transportation mechanism.

Specifically, JPEDET first employs a joint preference exploration module to fuse rating-based and review-based user representations, aiming to capture more robust and generalizable user preferences within each domain.
To bridge the embedding discrepancy across domains, the model introduces a dynamic embedding transportation module based on neural ordinary differential equations (Neural ODEs), which enables continuous and bidirectional transformation between source and target user embedding spaces.
Moreover, JPEDET incorporates optimal transport-based matching regularization and barycentric Wasserstein interpolation to guide the transportation trajectories, leading to smoother and more stable cross-domain alignment.

By modeling cross-domain adaptation as a dynamic and invertible embedding transformation process, JPEDET advances non-overlapping CDR beyond static embedding mapping or adversarial alignment methods, and demonstrates superior performance on multiple real-world benchmark datasets.
The overall framework of JPEDET can be illustrated as in Figure~\ref{fig:JPEDET}.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{figures/JPEDET.pdf}
	\caption{Illustration of the JPEDET framework~\cite{Liu_Chen_Liao_Hu_Tan_Wang_Zheng_Ong_2024_JPEDET}, which employs joint preference exploration and dynamic embedding transportation for non-overlapping cross-domain recommendation.}
	\label{fig:JPEDET}
\end{figure}

In addition to embedding-level alignment methods, several recent works explore cross-domain recommendation from a distributional perspective.
Liu et al.~\cite{Li_Qiu_Zhao_Wang_Zhang_Xing_Wu_2022_GWCDR} proposed GWCDR, which addresses non-overlapping cross-domain recommendation by aligning user-item interaction patterns at the distribution level.

Instead of relying on overlapped users or items, GWCDR constructs rating pattern distributions based on observed positive interactions, where each interaction is represented by the element-wise product of the corresponding user and item embeddings.
To bridge the discrepancy between domains without assuming a shared embedding space, the model employs Gromov--Wasserstein optimal transport to align the structural relationships within the interaction distributions of the source and target domains.

By preserving the relative geometry of interaction patterns across domains, GWCDR enables effective knowledge transfer under strict non-overlapping settings.
This distribution-level alignment paradigm provides a complementary perspective to embedding mapping, attribute transportation, and dynamic embedding transportation methods, and demonstrates strong performance across multiple real-world cross-domain recommendation benchmarks.
